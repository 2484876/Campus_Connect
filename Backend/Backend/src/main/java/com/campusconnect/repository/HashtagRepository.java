...
public interface HashtagRepository extends JpaRepository<Hashtag, Long> {
    ...
    @Query("SELECT h FROM Hashtag h ORDER BY h.usageCount DESC, h.lastUsed DESC")
    List<Hashtag> findTopTrending();

    @Query("SELECT h FROM Hashtag h WHERE h.lastUsed >= :since ORDER BY h.usageCount DESC")
    List<Hashtag> findTrendingSince(@Param("since") LocalDateTime since);

    @Query("SELECT h FROM Hashtag h WHERE LOWER(h.tag) LIKE LOWER(CONCAT(:q, '%')) ORDER BY h.usageCount DESC")
    List<Hashtag> searchByPrefix(@Param("q") String q);
}