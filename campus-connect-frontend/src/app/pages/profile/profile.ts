...
export class ProfileComponent implements OnInit {
  ...
  editForm: any = {};
  expForm: any = {
    title: '', employmentType: 'Full-time', company: 'Cognizant',
    ...
    this.route.params.subscribe(params => {
      ...
      this.api.getUser(id).subscribe({
        next: (user) => {
          ...
          this.isConnected = user.connectionStatus === 'ACCEPTED';
          this.loading = false;
          this.cdr.detectChanges();

          if (!this.isOwnProfile) {
            this.api.getMutualConnectionCount(id).subscribe({
              next: (count) => { this.mutualCount = count; this.cdr.detectChanges(); },
              error: () => { }
            });
          }
        },
  ...
  message(): void {
    if (this.user) this.router.navigate(['/chat', this.user.id]);
  }
  ...
  getAvatar(name: string): string {
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=2d5f3f&color=fff&size=120`;
  }
  ...
  toggleComments(post: PostDTO): void {
    ...
    if (post.showComments && (!post.recentComments || !post.recentComments.length)) {
      this.api.getComments(post.id).subscribe(r => {
        post.recentComments = r.content;
        this.cdr.detectChanges();
      });
    }
    this.cdr.detectChanges();
